    ...
    public partial class frmNhanKhau : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string ngaymat = dtpNgayMat.Checked ? $"'{dtpNgayMat.Value:yyyy-MM-dd}'" : "NULL";
                string id_que = cboQueQuan.SelectedValue != null ? cboQueQuan.SelectedValue.ToString() : "NULL";

                // SQL
                string sql = $@"INSERT INTO NhanKhau (soCCCD, hoTen, ngaySinh, gioiTinh, id_QueQuan, ngayMat, isDeleted) 
                                VALUES ('{txtCCCD.Text}', N'{txtHoTen.Text}', '{ngaysinh}', 
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string ngaysinh = dtpNgaySinh.Value.ToString("yyyy-MM-dd");
                string ngaymat = dtpNgayMat.Checked ? $"'{dtpNgayMat.Value:yyyy-MM-dd}'" : "NULL";
                string id_que = cboQueQuan.SelectedValue.ToString();

                string sql = $@"UPDATE NhanKhau 
                                SET soCCCD='{txtCCCD.Text}', hoTen=N'{txtHoTen.Text}', 
                                    ngaySinh='{ngaysinh}', gioiTinh=N'{cboGioiTinh.Text}', 
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DialogResult dr = MessageBox.Show("Bạn có chắc chắn muốn xóa nhân khẩu này?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dr == DialogResult.Yes)
                {
                    string sql = $"UPDATE NhanKhau SET isDeleted = 1 WHERE id = {txtID.Text}";
                    knn.ThucThi(sql);
                    Load_BangNhanKhau();
        ...
        private void dtaNhanKhau_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                // Đổ dữ liệu từ dòng vào các txt/cbo
